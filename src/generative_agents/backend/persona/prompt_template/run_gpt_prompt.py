...
def run_gpt_prompt_generate_hourly_schedule(
    ...
    def create_prompt_input(
        ...
        prompt_input += [intermission2] if intermission2 else [""]
...
def run_gpt_prompt_task_decomp(persona, task, duration, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        for count, i in enumerate(temp):
            _cr += (
                [" ".join([j.strip() for j in i.split(" ")][3:])] if count != 0 else [i]
            )
...
def run_gpt_prompt_action_game_object(
    ...
    def create_prompt_input(action_description, persona, temp_address, test_input=None):
        ...
        prompt_input += [
            persona.s_mem.get_str_accessible_arena_game_objects(temp_address)
        ]
        return prompt_input
...
def run_gpt_prompt_new_decomp_schedule(
    ...
    def __func_clean_up(gpt_response, prompt=""):
        new_schedule = f"{prompt} {gpt_response.strip()}"
    ...
    def __func_validate(gpt_response, prompt=""):
        try:
            ...
            x = (
                prompt.split("\n")[0]
                .split("originally planned schedule from")[-1]
                .strip()[:-1]
            )
            x = [
                datetime.datetime.strptime(i.strip(), "%H:%M %p")
                for i in x.split(" to ")
            ]
            delta_min = int((x[1] - x[0]).total_seconds() / 60)

            if int(dur_sum) != delta_min:
                return False
...
def run_gpt_prompt_decide_to_talk(
    ...
    def create_prompt_input(init_persona, target_persona, retrieved, test_input=None):
        ...
        if (
            len(init_persona.scratch.planned_path) == 0
            and "waiting" not in init_act_desc
        ):
        ...
        if (
            len(target_persona.scratch.planned_path) == 0
            and "waiting" not in init_act_desc
        ):
    ...
    def __func_validate(gpt_response, prompt=""):
        try:
            return gpt_response.split("Answer in yes or no:")[-1].strip().lower() in [
                "yes",
                "no",
            ]
        except Exception:
...
def run_gpt_prompt_decide_to_react(
    ...
    def create_prompt_input(init_persona, target_persona, retrieved, test_input=None):
        ...
        if len(target_persona.scratch.planned_path) == 0:
            ...
            target_p_desc = (
                f"{target_persona.name} is already {target_act_desc} at {loc}"
            )
        else:
            loc = ""
            if ":" in target_persona.scratch.act_address:
    ...
    def __func_validate(gpt_response, prompt=""):
        try:
            return gpt_response.split("Answer: Option")[-1].strip().lower() in [
                "3",
                "2",
                "1",
            ]
...
def run_gpt_prompt_create_conversation(
    ...
    def create_prompt_input(init_persona, target_persona, curr_loc, test_input=None):
        ...
        if init_persona.a_mem.seq_chat:
            for i in init_persona.a_mem.seq_chat:
                if i.object == target_persona.scratch.name:
                    v1 = int(
                        (init_persona.scratch.curr_time - i.created).total_seconds()
                        / 60
                    )
                    prev_convo_insert += (
                        f"{v1} minutes ago, they had the following conversation.\n"
                    )
        ...
        if prev_convo_insert == "\n":
            prev_convo_insert = ""
        if init_persona.a_mem.seq_chat and (
            int(
                (
                    init_persona.scratch.curr_time
                    - init_persona.a_mem.seq_chat[-1].created
                ).total_seconds()
                / 60
            )
            > 480
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        gpt_response = (
            (prompt + gpt_response).split("What would they talk about now?")[-1].strip()
        )
...
def run_gpt_prompt_summarize_conversation(
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ret = f"conversing about {gpt_response.strip()}"
        return ret
...
def run_gpt_prompt_extract_keywords(
    persona, description, test_input=None, verbose=False
):
...
def run_gpt_prompt_focal_pt(persona, statements, n, test_input=None, verbose=False):
    ...
    def __func_clean_up(gpt_response, prompt=""):
        gpt_response = f"1) {gpt_response.strip()}"
        ret = []
        for i in gpt_response.split("\n"):
            ret += [i.split(") ")[-1]]
        return ret
...
def run_gpt_prompt_agent_chat(
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        gpt_response = (
            (prompt + gpt_response).split("Here is their conversation.")[-1].strip()
        )
        content = re.findall('"([^"]*)"', gpt_response)
